...
esp_err_t pca9557_register_read(uint8_t reg_addr, uint8_t *data, size_t len) {
  ...
  esp_err_t ret = i2c_master_transmit_receive(
      pca9557_dev_handle, &reg_addr, 1, data, len, 1000 / portTICK_PERIOD_MS);
  if (ret != ESP_OK) {
...
esp_err_t pca9557_register_write_byte(uint8_t reg_addr, uint8_t data) {
  ...
  esp_err_t ret =
      i2c_master_transmit(pca9557_dev_handle, write_buf, sizeof(write_buf),
...
// 触摸屏初始化+添加LVGL接口
static lv_indev_t *bsp_display_indev_init_with_adapter(lv_disp_t *disp) {
  /* 初始化触摸屏 */
  ESP_ERROR_CHECK(bsp_touch_new(&tp));
  assert(tp);
...
void bsp_lvgl_start_with_adapter(void) {
  ...
  /* 初始化触摸屏 并添加LVGL接口 */
  disp_indev = bsp_display_indev_init_with_adapter(disp);
...
esp_err_t bsp_codec_set_fs(uint32_t rate, uint32_t bits_cfg,
  ...
  //     ret |= esp_codec_dev_set_in_gain(record_dev_handle,
