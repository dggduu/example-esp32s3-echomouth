...
static void task_process_ai(void *arg) {
  ...
  while (true) {
    ...
    if (gEvent) {
      ...
      //     esp_cpu_get_cycle_count(); // 获取开始时的 CPU 周期
      ...
      // // 打印推理性能
      // ESP_LOGI(TAG, "Inference Performance: Time: %.2f ms | CPU Cycles: %u",
      //          duration_ms, total_cycles);
...
static void task_process_lcd(void *arg) {
  ...
  while (true) {
    if (xQueueReceive(xQueueLCDFrame, &frame, portMAX_DELAY)) {
      lcd_draw_bitmap(0, 0, frame->width, frame->height,
                      (uint16_t *)frame->buf);
      esp_camera_fb_return(frame);
    }
  }
...
void app_camera_ai_lcd(void) {
  ...
  xTaskCreatePinnedToCore(task_process_lcd, "task_process_lcd", 4 * 1024, NULL,
                          5, NULL, 0);
  xTaskCreatePinnedToCore(task_process_ai, "task_process_ai", 4 * 1024, NULL, 5,
                          NULL, 0);
}
