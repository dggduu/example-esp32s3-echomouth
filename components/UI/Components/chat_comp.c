...
static void render_window(void) {
  ...
  if (last_visible_idx >= 0) {
    ...
    lv_obj_scroll_to_view(s_wrappers[last_visible_idx], LV_ANIM_OFF);
  }
}
...
lv_obj_t *chat_comp_create(lv_obj_t *parent) {
  ...
  for (int i = 0; i < CHAT_WINDOW_SIZE; i++) {
    ...
    /* 每一个 wrapper 也是一个 Row 方向 Flex 容器 */
    ...
    lv_obj_clear_flag(s_wrappers[i], LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_flag(s_wrappers[i], LV_OBJ_FLAG_HIDDEN);
    ...
    lv_obj_clear_flag(s_bubbles[i], LV_OBJ_FLAG_SCROLLABLE);
  ...
  /* 4. 圆形屏左右侧按弧度计算的 FAB 按钮分布 */
  int16_t left_btn_h = 38;
  int16_t left_exit_y = (UI_SCREEN_HEIGHT - left_btn_h) / 2;
  int16_t left_exit_pad = ui_circle_get_safe_pad(left_exit_y, left_btn_h) + 6;
  ...
  int16_t gap = 6;
  int16_t fab1_y = (UI_SCREEN_HEIGHT - (38 * 3 + gap * 2)) / 2;
  ...
  int16_t ta_y = UI_SCREEN_HEIGHT - ta_bottom_offset - ta_height;
