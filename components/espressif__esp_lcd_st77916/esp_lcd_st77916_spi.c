...
static esp_err_t panel_st77916_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
                                           int y_start, int x_end, int y_end,
                                           const void *color_data);
static esp_err_t panel_st77916_invert_color(esp_lcd_panel_t *panel,
                                            bool invert_color_data);
static esp_err_t panel_st77916_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                      bool mirror_y);
...
esp_err_t esp_lcd_new_panel_st77916_spi(
  ...
  ESP_GOTO_ON_FALSE(st77916, ESP_ERR_NO_MEM, err, TAG,
                    "no mem for st77916 panel");
...
static esp_err_t tx_param(st77916_panel_t *st77916,
  ...
  esp_err_t ret = esp_lcd_panel_io_tx_param(io, lcd_cmd, param, param_size);
...
// static esp_err_t tx_color(st77916_panel_t *st77916,
//                           esp_lcd_panel_io_handle_t io, int lcd_cmd,
//                           const void *param, size_t param_size) {
//   if (st77916->flags.use_qspi_interface) {
//     lcd_cmd &= 0xff;
...
static esp_err_t tx_color(st77916_panel_t *st77916,
  ...
  esp_err_t ret = esp_lcd_panel_io_tx_color(io, lcd_cmd, param, param_size);

  // 等待发送完成
  esp_lcd_panel_io_tx_color(io, -1, NULL, 0);
...
static esp_err_t panel_st77916_reset(esp_lcd_panel_t *panel) {
  ...
  // ===== 使用 PCA9539 Reset =====
  bsp_pca9539_set_pin_level(BSP_IOEXP_LCD_RST, st77916->flags.reset_level);
...
static const st77916_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {0xE0,
     (uint8_t[]){0xF0, 0x10, 0x18, 0x0D, 0x0C, 0x38, 0x3E, 0x44, 0x51, 0x39,
                 0x15, 0x15, 0x30, 0x34},
     14, 0},
    {0xE1,
     (uint8_t[]){0xF0, 0x0F, 0x17, 0x0D, 0x0B, 0x07, 0x3E, 0x33, 0x51, 0x39,
                 0x15, 0x15, 0x30, 0x34},
     14, 0},
...
static esp_err_t panel_st77916_init(esp_lcd_panel_t *panel) {
  ...
  if (st77916->init_cmds) {
    ...
    init_cmds_size =
        sizeof(vendor_specific_init_default) / sizeof(st77916_lcd_init_cmd_t);
  }
  ...
  for (int i = 0; i < init_cmds_size; i++) {
    ...
    // Send command
    ESP_RETURN_ON_ERROR(tx_param(st77916, io, init_cmds[i].cmd,
...
static esp_err_t panel_st77916_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
  ...
  size_t len =
      (x_end - x_start) * (y_end - y_start) * st77916->fb_bits_per_pixel / 8;
...
static esp_err_t panel_st77916_disp_on_off(esp_lcd_panel_t *panel,
                                           bool on_off) {
  st77916_panel_t *st77916 = __containerof(panel, st77916_panel_t, base);
  esp_lcd_panel_io_handle_t io = st77916->io;
  int command = 0;
